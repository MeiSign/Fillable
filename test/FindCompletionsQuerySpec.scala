...
class FindCompletionsQuerySpec extends Specification {
  ...
  "FindCompletionsQuery Constructor" should {
    ...
    "throw an Illegal Argument Exception for a null toBeCompleted text" in new WithApplication {
      new FindCompletionsQuery("test", null) must throwA[IllegalArgumentException]
    }
    
    "throw an Illegal Argument Exception for a null indexName text" in new WithApplication {
      new FindCompletionsQuery(null, "test") must throwA[IllegalArgumentException]
    }
    
    "getUrlAddon must return the correct url Addon" in new WithApplication {
      new FindCompletionsQuery("index", "test").getUrlAddon must beEqualTo("/index/_suggest")
    }
    
    "toJson should return the correct Json object" in new WithApplication {
      new FindCompletionsQuery("indexName", "text").toJson must beEqualTo(
        Json.obj(
          "indexName" -> Json.obj(
            "text" -> "text",
    ...
    "query must be of http type post" in new WithApplication {
      new FindCompletionsQuery("test", "index").httpType must beEqualTo(HttpType.Post)
    }
  }
  ...
  "FindCompletionsQuery getResult" should {
    "throw an Illegal Argument Exception for a null response getResult call" in new WithApplication {
      new FindCompletionsQuery("Test", "test").getResult(null) must throwA[IllegalArgumentException]
    }
    ...
    "should return the correct completions for a valid json response" in new WithApplication {
      ...
      new FindCompletionsQuery("Test", "test").getResult(response) must beEqualTo(Json.parse("""{"status":"ok","completions":[{"text":"Hotel Marriot","score":15.0},{"text":"Hotel Monaco","score":10.0},{"text":"Hotel Mercure","score":5.0}]}""").as[JsObject])
    }
    
    "should return an error object for invalid json responses" in new WithApplication {
      val response: JsValue = Json.parse("""{"error" : "bla"}""")
      new FindCompletionsQuery("Test", "test").getResult(response) must beEqualTo(Json.parse("""{"status":"error", "msg" : "bla"}""").as[JsObject])
    }
    ...
    "should return an empty result for a valid empty json response" in new WithApplication {
      ...
      new FindCompletionsQuery("Test", "test").getResult(response) must beEqualTo(Json.parse("""{"status":"ok","completions":[]}""").as[JsObject])
    }
  }
