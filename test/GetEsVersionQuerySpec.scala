...
class GetEsVersionQuerySpec extends Specification {
  ...
  "GetEsVersionQuery getResult" should {
    ...
    "respondError with correct error msg if es version is not accessible" in new WithApplication {
      val esStatus = Json.parse("""{"ok" : true, "status" : 200, "name" : "Payge, Reeva", "tagline" : "You Know, for Search" }""")
      new GetEsVersionQuery().getResult(esStatus) must beEqualTo(Json.parse("""{"status":"error","msg":"Could not determine elasticsearch version."}"""))
    }
  }  
}